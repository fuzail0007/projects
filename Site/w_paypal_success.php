...
<?php
		...
		//this is for customer details
			...
			$insert_payment = "insert into wpayments (amount,customer_id,product_id,trx_id,currency,payment_date) values ('$amount','$c_id','$pro_id','$trx_id','$currency',NOW())";
			
			$run_payments = mysqli_query($con, $insert_payment);
			
			//inserting the order into table
			$insert_order = "insert into worders (p_id, c_id, qty, invoice_no, order_date,status) values ('$pro_id','$c_id','$qty','$invoice',NOW(),'In Progress')";
			$run_order = mysqli_query($con, $insert_order);
			
			//Removing the product from cart after order takes place 
			$empty_cart = "delete from cart2";
			$run_cart = mysqli_query($con, $empty_cart);
			
			if($amount==$total)
			{
				echo "<h2>Welcome:" . $_SESSION['customer_email']. "<br>" . "Your payment was Successful!</h2>";
				echo "<a href = 'http://localhost/mysitee/customer/my_account.php'>Go to Your Account</a>";
			}
			else
			{
				echo "<h2>Sorry Your Transaction was Failed!</h2>";
				echo "<a href = 'http://localhost/mysitee'>Go Back</a>";
			}
	...
	<h2> <a href='http://localhost/mysite'>Click here</a> to login to your account.</h2>
	
	<h3> Thank you for your order @ - www.theleathercity.com</h3>
